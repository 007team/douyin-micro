...
func CheckUserExist(user *models.User) (err error) {
	var count int64
	err = db.Model(&models.User{}).Distinct("name").Where("name = ?", user.Name).Count(&count).Error
	if err != nil {
		log.Println("mysql.CeckUserExist  用户数据查询失败", err)
		return err
	}
	if count > 0 {
...
func CreateNewUser(user *models.User) (err error) {
	if err = db.Select("Name", "Password", "Salt").Create(user).Error; err != nil {
		log.Println("mysql.CreateNewUser  创建新用户失败", err)
		return err
	}
	return
}
...
func UserInfo(user *models.User) (err error) {
	...
	if err = db.First(user, user.Id).Error; err != nil {
		log.Println("mysql.UserInfo 查询错误", err)
		return err
	}
